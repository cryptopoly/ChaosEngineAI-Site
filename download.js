...
(() => {
  const REPO = 'cryptopoly/ChaosEngineAI';
  const RELEASES_PAGE = `https://github.com/${REPO}/releases`;
  const LATEST_PAGE = `https://github.com/${REPO}/releases/latest`;
  const API_LATEST = `https://api.github.com/repos/${REPO}/releases/latest`;

  // Asset matchers — first regex that matches an asset.name wins.
  const PLATFORMS = [
    {
      id: 'mac-arm64',
      label: 'macOS',
  ...
  // --- Platform detection from userAgent -----------------------------------
  ...
  // Wire up any element with data-download or href="#install" (the hero CTA).
  function wire() {
